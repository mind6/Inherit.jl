...
function createshadowmodule(basemodule::Module)::Module
	...
	shadowmod = Base.invokelatest(getproperty, basemodule, H_SHADOW_SUBMODULE)
	...
	for name in names(basemodule; imported=true, all=true)
		if name == H_SHADOW_SUBMODULE || !isdefined(basemodule, name)
			continue
		end
		p = Base.invokelatest(getproperty, basemodule, name)
		if p isa Function
	...
	for name in names(basemodule; imported=true, all=true)
		...
		if p isa Union{Type,Module} && p ∉ functypes
			...
			if isdefined(shadowmod, name)
				Base.invokelatest(setproperty!, shadowmod, name, p)
			else
...
function cleanup_shadowmodule(shadowmod::Module)
	...
	for name in names(shadowmod; imported=true, all=true)
		try
			Base.invokelatest(setproperty!, shadowmod, name, nothing)
			cleaned += 1
		catch e
			errored += 1
		end
...
function import_symbol_into_shadowmodule(current_module::Module, shadowmodule::Module, symbol::Symbol)
	if isdefined(current_module, symbol)
		p = Base.invokelatest(getproperty, current_module, symbol)
		if isdefined(shadowmodule, symbol)
			Base.invokelatest(setproperty!, shadowmodule, symbol, p)
		else
...
function find_supertype_module(currenttype::Type{T}, identS::TypeIdentifier)::Module where T
	...
	if identS.modulefullname == fullname(ctmod) && identS.basename == ctname
		...
		if st === Any
			error("requested identS $identS is not a supertype of discovered runtime type $currenttype")
		end
